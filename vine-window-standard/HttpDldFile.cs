...
namespace vine_window_standard
{
    class HttpDldFile
    {
        ...
        {
            //目标路径
            string subPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + "\\vine-windows-standard\\Update";
            //下载文件到文件夹
            ...
            if (!File.Exists(subPath + "\\" + fileName))
                File.Delete(subPath + "\\" + fileName);
            flag = Download(url, subPath + "\\" + fileName);
            return flag;
        }
